...
def rewrite_query(query: str, history: list[dict]) -> str:
    ...
    prompt = (
        ...
        "독립적인 한국어 질문으로 재작성해. "
        "지시어(그거, 이거, 아까 등)는 직전 '사용자' 질문이 묻고 있던 핵심 대상"
        "(보통 컬럼명·용어명 등 질문의 주어)으로 바꿔 — 직전 'AI' 답변 안에 등장한 "
        "부가 정보(약어, 코드, 도메인명 등 답의 세부 내용)로 바꾸면 안 돼. "
        "예) Q:'주민등록번호 데이터타입 알려줘' A:'CHAR입니다. 영문약어는 RRNO...' "
        "다음 질문 '그럼 영문약어는?' → 올바른 재작성: '주민등록번호의 영문약어는 뭐야?' "
        "(틀린 예: 'RRNO는 무엇인가요?' — RRNO는 답변 속 세부값이지 질문 대상이 아님) "
        "재작성된 질문만 출력해, 설명 없이.\n\n"
        f"대화 맥락:\n{dialogue}\n\n"
        f"재작성할 질문: {query}"
    )
