...
- 가독성을 위해 핵심 값은 마크다운 굵게(`**텍스트**`)로 강조합니다: 컬럼명/용어명,
  영문약어, 데이터타입, 길이·자릿수, 그리고 결론 문장의 핵심 답. 문장 전체나
  일반 설명문까지 굵게 만들지 말고, 사용자가 훑어봤을 때 바로 눈에 띄어야 할
  값·용어에만 선택적으로 적용합니다.
...
def run(query: str, conv_id: Optional[str] = None) -> dict:
    ...
    if subject and final_docs and not has_exact_subject_match:
        no_exact_hit_warning = (
            ...
            "먼저 근거를 검토해 '정의가 없다'는 결론에 도달했다면, 답변의 맨 앞 결론"
            "(데이터타입/길이 등)도 반드시 그와 똑같이 '정의되어 있지 않음'으로 써야 "
            "합니다 — 근거에서는 없다고 판단하고 맨 앞 결론에서는 그와 다르게 구체적인 "
            "타입/길이를 제시하는 자기모순은 절대 허용되지 않습니다."
        )
